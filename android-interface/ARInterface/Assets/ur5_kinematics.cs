...
public class ur5_kinematics : MonoBehaviour {
    ...
    void TaskOnClick3()
    {
        ...
        Debug.LogFormat("Sphere location is:  {0}", targetPos);
        // UPDATE THIS

        var numTouches = Input.touchCount;
        while (Input.touchCount == numTouches) ;

        var particle =  new GameObject();
        foreach (var touch in Input.touches) {
            if (touch.phase == TouchPhase.Began) {
                outputText.text = touch.position.ToString();
    ...
    public class ur5
    {
        ...
        // Helper function to convert from xyz coords & rot into 4x4 pose matrix
        // Input in meters & radians
        public Matrix<float> format_pose(float x_pos, float y_pos, float z_pos, float x_rot, float y_rot, float z_rot) {
            ...
            pose[0, 1] = Mathf.Cos(roll) * Mathf.Sin(pitch) * Mathf.Sin(yaw) - Mathf.Cos(yaw) * Mathf.Sin(roll);
            pose[0, 2] = Mathf.Sin(roll) * Mathf.Sin(yaw) + Mathf.Cos(roll) * Mathf.Cos(yaw) * Mathf.Sin(pitch);
            //pose[0, 3] = 0f;

            pose[1, 0] = Mathf.Cos(pitch) * Mathf.Sin(roll);
            pose[1, 1] = Mathf.Cos(roll) * Mathf.Cos(yaw) + Mathf.Sin(pitch) * Mathf.Sin(roll) * Mathf.Sin(yaw);
            pose[1, 2] = Mathf.Cos(yaw) * Mathf.Sin(pitch) * Mathf.Sin(roll) - Mathf.Cos(roll) * Mathf.Sin(yaw);
            //pose[1, 3] = 0f;

            pose[2, 0] = -1f * Mathf.Sin(pitch);
        ...
        // Helper function to convert from UNITY coords & rot into 4x4 pose matrix
        // Input in meters & radians
        // Note that this accounts for Unity's x-y-z rotational scheme vs z-y-x above
        public Matrix<float> format_pose_Unity(float x_pos, float y_pos, float z_pos, float x_rot, float y_rot, float z_rot) {
            Matrix<float> pose = Matrix<float>.Build.Dense(4,4);
            ...
            pose[2, 1] = -1f * Mathf.Cos(pitch) * Mathf.Sin(yaw);
            pose[2, 2] = Mathf.Cos(pitch) * Mathf.Cos(yaw);
